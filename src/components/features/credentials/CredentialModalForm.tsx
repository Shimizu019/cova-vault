...
import { Eye, EyeOff, RefreshCw, FolderOpen, Plus } from "lucide-react";
...
interface CredentialFormProps {
  ...
  onSubmit: (data: { name: string; username: string; password: string; website: string; tags: string[]; folderId?: string }) => void;
}

export function CredentialForm({ initial, onSubmit }: CredentialFormProps) {
  const { folders, addFolder } = useCredentialStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setErrors({});
    onSubmit({ name, username, password, website, tags, folderId: folderId || undefined });
  };
  ...
  return (
    <>
      <form id="cred-form" onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <Input id="cred-username" type="email" value={username}
            onChange={(e) => setUsername(e.target.value)} placeholder="username@email.com" error={errors.username} />
        </div>
        <div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select id="cred-folder" value={folderId} onChange={(e) => setFolderId(e.target.value)} className="input flex-1">
              <option value="">No Folder</option>
              {folders.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}
            </select>
            <Button type="button" variant="secondary" onClick={() => setShowFolderModal(true)} title="Create new folder">
              <Plus className="w-4 h-4" />
            </Button>
          </div>
          {selectedFolder && (
            <p className="text-xs text-cova-textMuted mt-1 flex items-center gap-1">
              <FolderOpen className="w-3 h-3" /> {selectedFolder.name}
            </p>
          )}
        </div>
        <div>
            ...
            {tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-cova-primary/15 text-cova-primary text-xs font-medium">
                ...
                <button type="button" onClick={() => setTags(tags.filter((t) => t !== tag))}
                  className="hover:opacity-70" aria-label={`Remove tag ${tag}`}>&times;</button>
              </span>
            ))}
          </div>
      ...
      <Modal isOpen={showFolderModal} onClose={() => { setShowFolderModal(false); setNewFolderName(""); }}
        title="Create Folder" size="sm" footer={<><Button variant="secondary" onClick={() => { setShowFolderModal(false); setNewFolderName(""); }}>Cancel</Button><Button variant="primary" onClick={handleCreateFolder}>Create &amp; Select</Button></>}>
        <div>
          <Label htmlFor="new-folder-name" required>Folder name</Label>
          <Input id="new-folder-name" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)}
            placeholder="e.g. Work accounts" autoFocus />
        </div>
      </Modal>
    </>
  );
